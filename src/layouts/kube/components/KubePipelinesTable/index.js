...
function KubePipelinesTable(props) {
  const { columns, rows } = parseActionsData(props, props.triggerKubeDetailModal);
  ...
  return (
    ...
    >
          ...
          <GuiBox mt="8px" mb="auto" display="flex" sx={{ gap: "8px" }}>
            <GuiBox display="flex" alignItems="center" lineHeight={0}>
              ...
              <GuiTypography variant="button" fontWeight="regular" color="text" ml="5px">
                &nbsp;<strong>{props.data.length === 1 ? props.data.length + " pipeline" : props.data.length + " pipelines"}</strong>
              </GuiTypography>
            </GuiBox>
          </GuiBox>
        </GuiBox>
      ...
      >
        <Table columns={columns} rows={rows} itemsPerPage={20}/>
      </GuiBox>
      {/* :
        <></>
      } */}
...
export default KubePipelinesTable;
