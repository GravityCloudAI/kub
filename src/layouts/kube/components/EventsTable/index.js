...
// Data
import parseEventsData from "layouts/services/components/EventsTable/data";
import { EventAvailableOutlined, EventSeat } from "@mui/icons-material";
import { SiEventstore } from "react-icons/si";
import { MdDataUsage } from "react-icons/md";

function EventsTable(props) {
  const { columns, rows } = parseEventsData(props);
  ...
  return (
    ...
    >
      <GuiBox display="flex" justifyContent="space-between" alignItems="center" mb="8px" sx={{ cursor: "pointer" }}>
        <GuiBox mb="auto">
          <GuiTypography color="white" variant="lg" mb="6px" gutterBottom>
            {props?.title}
          </GuiTypography>
          <GuiBox display="flex" alignItems="center" lineHeight={0}>
            <MdDataUsage color="green" size="15px" />
            <GuiTypography variant="button" fontWeight="regular" color="text" ml="5px">
              &nbsp;<strong>Past {rows.length} events</strong>
            </GuiTypography>
          </GuiBox>
        </GuiBox>
