...
// Data
import parseActionsData from "layouts/kube/components/KubeEventsTable/data";

function KubeEventsTable(props) {
  ...
  return (
    ...
    >
          ...
          <GuiBox mt="8px" mb="auto" display="flex" sx={{ gap: "8px" }}>
            <GuiBox display="flex" alignItems="center" lineHeight={0}>
              ...
              <GuiTypography variant="button" fontWeight="regular" color="text" ml="5px">
                &nbsp;<strong>{props.data.length === 1 ? props.data.length + " event" : props.data.length + " events"}</strong>
              </GuiTypography>
            </GuiBox>
          </GuiBox>
        </GuiBox>
      ...
      >
        <Table columns={columns} rows={rows} itemsPerPage={5}/>
      </GuiBox>
      {/* :
        <></>
      } */}
...
export default KubeEventsTable;
